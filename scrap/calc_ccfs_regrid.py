...
Note: Outdated, please see calculate_Tadv.py
Output computed with taux and tauy substitutions have generally been moved to "calc_with_tauxy" folders
...
# Options
regrid   = False # Set to True to use 1x1 regridded output
calc_ws  = False # Set to True to calculate Wind Speed
vnames   = ['sst','u10','v10']#["tx_sur","ty_sur","sst"] #Dont change order

# Load Land Mask
landmask = ut.load_land_mask_awi("TCo319",regrid=regrid)
expnames = ["TCo1279-DART-1950",]#["TCo2559-DART-1950C",] #["TCo1279-DART-1950","TCo2559-DART-1950C"]
...
for v in range(3):
    ...
    if regrid:
        ncname = "%s%s_%s_regrid1x1.nc" % (datpath,expname,vnames[v])
    else:
...
for ex in range(nexp):
    ...
    for v in range(3):
        if regrid:
            ...
            ncname = "%s%s_%s.nc" % (datpath,expnames[ex],vnames[v])
        ds = xr.open_dataset(ncname)[vnames[v]]#.load()
...
#%% Calculation of advection (modified to copy [calculate_ccfs_era5.py])
...
for ex in range(nexp):
    ...
    #dtday     = 3600*24
    #(Tadv2mean*dtday).plot(vmin=-.1,vmax=.1,cmap='cmo.balance'),plt.show()
    
    ds_Tadv.append(Tadv2)


#%% Save Output

vnames_out = ["ws10","Tadv"]
dsouts     = [dsws,ds_Tadv]
...
#%% More Debuggin Visualizations

Tadvmeans = [T.mean('time') for T in ds_Tadv]
(Tadvmeans[0]*dtday).plot(vmin=-.1,vmax=.1,cmap='cmo.balance'),plt.show()
...
# #%%  ==============================================================
# #%% Section below has been moved to "calculate_radiative_kernels"
...
# #%% Section above has been moved to "calculate_radiative_kernels"
# #%% Scrap below ==============================================================
