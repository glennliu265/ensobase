...
#%% Quick Individual plots to see the rough color ranges
...
for ex in range(nexps):
    ...
    ax.set_title("%s (%s)" % (vname,expnames_long[ex]))
    plt.show()
    
#%% Do a draft plot

#cints    = np.arange(18,35.5,.5)
cints     = np.arange(350,384,2)#np.arange(120,290,10)  #np.arange(-100,110,10)#np.arange(0,0.95,0.05)
fig,axs   = ut.init_tp_map(2,3,figsize=(24,4.5))

plotorder = [0,2,4,1,3]
cmaptest  = 'plasma_r'#'cmo.solar'#"plasma_r"
...
#%% "Final" Plot (still very much a draft)
...
for im in range(12):
    ...
    savename = "%sMeanState_Plot_AWI_%s_Combined_mon%02i.png" % (figpath,vname,im+1)
    plt.savefig(savename,dpi=150,)#bbox_inches='tight')
    #plt.show()
